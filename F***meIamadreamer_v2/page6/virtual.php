...
<head>
    <title><?php echo $titrePage6 ?></title>
    <meta charset="utf-8">
...
?>
    ...
    <section>
        <div class="container">
            ...
            <?php
            $index = 0;
            foreach ($productsvirtual as $k => $v)
                {
                $titreCard = $k;
    ...
    <section>
        <div class="container">
            ...
            $products1virtual = array_intersect_key($productsvirtual, array_flip(array_rand($productsvirtual, 3))); 

            $index = $index + 1;
            foreach ($products1virtual as $k => $v)
                {
                $titreCard = $k;
                $imagePresentation = $v[0];
                $description = $v[1];
    ...
    <section>
        <div class="container">
            ...
            <?php
            //Trouvé sur le net car chaud :
            $products2virtual = array_intersect_key($productsvirtual, array_flip(array_rand($productsvirtual, 3))); 

            $index = $index + 1;
            foreach ($products2virtual as $k => $v)
                {
                $titreCard = $k;
                $imagePresentation = $v[0];
                $description = $v[1];
